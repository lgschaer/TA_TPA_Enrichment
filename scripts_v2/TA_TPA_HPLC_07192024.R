...
#load hplc data
hplc <- read_csv("/Users/lgschaer/Downloads/HPLC_TPA_TA_Experiment.csv")
head(hplc)
...
tpa <- hplc2 %>% filter(Substrate == "Terephthalate") %>%
  mutate(
    locLab = case_when(
      Location == "Control" ~ "Starting\nConc.",
      Location == "Farm Compost" ~ "Compost",
      Location == "Hydrocarbon Seep" ~ "Hydrocarbon\nSeep",
      Location == "Lake Sediment" ~ "Lake\nSediment",
      Location == "Shoreline Sediment" ~ "Shoreline\nSediment",
      Location == "Stream Sediment" ~ "Stream\nSediment"),
    locLab = factor(locLab, levels = c("Starting\nConc.", "Compost", "Hydrocarbon\nSeep", "Lake\nSediment", "Shoreline\nSediment", "Stream\nSediment"))) %>%
  group_by(Location, locLab, Substrate) %>%
  summarise(
    maxConc = max(Concentration_g_L),
    meanConc = mean(Concentration_g_L),
    minConc = min(Concentration_g_L)
  )
head(tpa)
#unique(tpa$Location)
...
ggplot(tpa, aes(x = locLab, y = meanConc))+
  geom_col(aes(fill = Substrate), color = "black")+
  geom_errorbar(aes(ymin=minConc, ymax=maxConc), width=.5)+
  scale_fill_manual(values = c("olivedrab4"))+
  ylab("Terephthalate (g/L)")+
  theme_bw(base_size = 14)+
  theme(axis.text.y.left = element_text(size = 15, color = "black"),
