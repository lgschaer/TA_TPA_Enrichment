...
enriched_filt3 <- enriched_filt %>%
  filter(Enriched_Genus == "Rhodococcus" |
           Enriched_Genus == "Arthrobactor" |
           Enriched_Genus == "Bradyrhizobium" |
           Enriched_Genus == "Pseudomonas" |
           Enriched_Genus == "Variovorax" |
           Enriched_Genus == "Hydrogenophaga" |
           Enriched_Genus == "Pseudoxanthomonas")
head(enriched_filt3)

ggplot(data=enriched_filt3, aes(y=Taxonomy, x=log2FoldChange)) +
  facet_nested(rows = vars(Comparison), space = "free_y", scales = "free_y")+
  ...
  theme(axis.text.x = element_text(size = 30, angle = 0, vjust = 1, hjust = 0.5),
        ...
        legend.title = element_blank(),
        #strip.text.x = element_text(size = 40, face = "bold", angle = 0),
        strip.text.y = element_text(size = 35, face = "bold", angle = 270),
        legend.position = "bottom",
        title = element_text(size = 18))
