...
sdata3 <- sdata %>%
  ...
  mutate(rownames = SampleID,
         Substrate_Label = case_when(
           Substrate == "Control" ~ "Control",
           Substrate == "TA" ~ "Terephthalamide",
           Substrate == "TPA" ~ "Terephthalate"),
         Environment_Label_Location = case_when(
           Environment == "Compost" ~ "Compost\nCalumet, MI",
           Environment == "Sediment1" ~ "Lake Sediment\nBete Grise, MI",
           Environment == "Sediment2" ~ "River Sediment\nMichagamme, MI",
           Environment == "Sediment3" ~ "Shoreline Mud\nSt. Ignace, MI",
           Environment == "Sediment4" ~ "Stream Sediment\nFall Run, PA",
           Environment == "Sediment6" ~ "Hydrocarbon Seep\nCaspian Sea"),
         Environment_Label = case_when(
           Environment == "Compost" ~ "Compost",
           Environment == "Sediment1" ~ "Lake Sediment",
           Environment == "Sediment2" ~ "River Sediment",
           Environment == "Sediment3" ~ "Shoreline Mud",
...
pps<- rarefy_samplesover1000_all %>%
  subset_samples(Transfer != "Inocula")
pps
...
random_tree = ape::rtree(ntaxa(pps_filt), rooted=TRUE, tip.label=taxa_names(pps_filt))
plot(random_tree)
...
class <- all %>%
  group_by(Sample_Type, Substrate, Transfer, Class)%>%
